...
function get_possible_colors(col) {
  ...
  // let possible_hue_transforms = [1,2,3,5,7,10,180]
...
function setup() {
  ...
  // seed pixel acceptable hues
  hues = [random_int(0,360)]
  // add some selections of various transformations
  for(let i = 0; i < 3; i++){
    hues.push(hues[0]+randomChoice([5,10,15,0,0,0,0,0,0,0,0,0,45,45,45,120,120,180,180,180,180,270,270])%360)
  }

  // get the number of unique hues
...
function renew_pixels(){
  // set all pixels to unseen
  for (let i = 0; i < wth; i++) {
    for (let j = 0; j < hgt; j++) {
      pixeldata[i][j].state = "unseen"
      pixeldata[i][j].colors = [randomChoice(pixeldata[i][j].colors)]
    }
  }
  // get a new hash
  ...
  // set the random seed
  fxrand = sfc32(...hashes)
  // get a new entropy locking method
  locking_method = randomChoice(ent_lock_methods)
