...
colors = {pal1,pal2,pal3,pal4,pal5}
pal_to_use = 1
pal_tracker = 0
...
::_::
 ...
 if srf then
  srand(seed)
  pal_change_needed = true
 elseif pal_change_needed then
  pal_change_needed = false
  pal_tracker += 1
  pal_to_use = (pal_tracker % #colors) + 1
 end
 ...
 for i=0,100 do
  ...
  pset(x*flr(rnd(3)+1),y,9)
  pset(-x*flr(rnd(3)+1),y,9)
  -- pset(x*flr(rnd(3)+1),-y,9)
  pset(-x*flr(rnd(3)+1),-y,9)

  pset(x/flr(rnd(3)+1),y*i,9)
  -- pset(-x/flr(rnd(3)+1),y*i,9)
  -- pset(x/flr(rnd(3)+1),-y*i,9)
  pset(-x/flr(rnd(3)+1),-y*i,9)
